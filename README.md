```markdown
# Моделирование и оптимизация маршрутов

Проект содержит два независимых решения задачи поиска **кратчайшего пути** в ориентированном взвешенном графе, где:
- **Вершины** — населённые пункты, обозначенные заглавными латинскими буквами (`A`, `B`, `C`, …);
- **Рёбра** — дороги с заданной длиной в километрах;
- При **равных расстояниях** предпочтение отдаётся маршруту с **меньшим числом промежуточных пунктов**.

Реализации:
- 🐍 `ok.py` — Python-скрипт с алгоритмом Дейкстры;
- 📊 `ok.xlsx` — Excel-модель с использованием надстройки **«Поиск решения»**.
...
> ⚠️ **Примечание**: В текущей реализации **не учитывается** требование о выборе пути с меньшим числом вершин при равных расстояниях. Для полного соответствия ТЗ необходимо расширить алгоритм Дейкстры (например, хранить пару `(расстояние, количество шагов)` в очереди).
...
> `Файл` → `Параметры` → `Надстройки` → Управление: **«Надстройки Excel»** → `Перейти` → отметьте **«Поиск решения»**.

### 📂 Структура файла
- **Таблица смежности (10×10)** — содержит длины дорог между пунктами `A`–`J`:  
  - Пустая ячейка = отсутствие дороги;  
  - Число = расстояние в км.
- **Вспомогательная таблица** — бинарные переменные (`0`/`1`), указывающие, используется ли ребро.
- **Целевая ячейка** — суммарное расстояние (минимизируется).
- **Столбец «Путь»** — формирует маршрут (например, `ADHIJ`) с помощью функций `ЕСЛИ` и `СЦЕПИТЬ`.
...
- Нельзя войти/выйти из одной вершины более одного раза;
- Нельзя использовать несуществующие дороги (ограничение по исходной таблице).

### ▶️ Использование
1. Откройте `ok.xlsx`;
2. Убедитесь, что граф задан корректно в таблице смежности (`A`–`J`);
3. Перейдите на вкладку **«Данные» → «Поиск решения»**;
4. Нажмите **«Найти решение»**;
5. Результат появится в:
